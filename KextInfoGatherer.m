...
// PHT        [dict setObject:[NSString stringWithFormat:@"%-18p",loadAddressValue] forKey:@"Address"];
        [dict setObject:[NSString stringWithFormat:@"%-18llu",loadAddressValue] forKey:@"Address"];

        if (runningKernelArch->cputype & CPU_ARCH_ABI64) {
            if (loadAddressValue == (uint64_t)-1) {
    // PHT [dict setObject:[NSString stringWithFormat:@"%%-18s", kStringInvalidLong] forKey:@"Address"];
            ...
            if (loadAddressValue == (uint64_t)-1) {
    // PHT [dict setObject:[NSString stringWithFormat:@"%%-10s",kStringInvalidLong] forKey:@"Address"];
...
// PHT
// PHT End restored sorce code
// PHT
@end

// PHT this is what the source code looked like before retoration
